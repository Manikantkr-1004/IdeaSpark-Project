...
export default function PublicIdeas() {
    ...
    const [ideaData, setIdeaData] = useState<ideaPublicDataType[]>([]);
    ...
    const fetchPublicIdeas = async () => {
        ...
        try {
            const response = await axios.get(`${BACKEND_API}/idea/public?heading=${query.heading}&category=${query.category}`);
            const data: ResponseType = response.data;
            setIdeaData(data?.data);
        } catch (error: any) {
            console.log('Error is public idea fetch', error);
                            ...
                            ideaData?.map((ele: ideaPublicDataType) => (
                                <div key={ele._id} className="p-2 animate__animated animate__zoomIn self-start border relative border-(--light-color) bg-(--light-color)/25 rounded-md">
                                    ...
                                    <div className="flex items-center gap-3 text-sm font-semibold text-slate-800 mt-2">
                                        Published by {ele.authorId.name} <Image className="w-5 h-5 rounded-full bg-white border border-slate-800" src={ele.authorId.profileImg} alt={ele.authorId.name} width={20} height={20} />
                                    </div>
